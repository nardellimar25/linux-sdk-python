...
class FrameReceiver(threading.Thread):
    """
    Receives JPEG frames (raw or blurred) via UDP, decodes them, saves a debug image, 
    and puts the frame into the provided queue.
    """
    def __init__(self, ip, port, frame_queue, receiver_type, debug_path, barrier, process_delay=0.01, buffer_size=65535):
...
class CoordsReceiver(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            try:
                data, _ = self.sock.recvfrom(self.buffer_size)
                coords_data = json.loads(data.decode('utf-8'))
